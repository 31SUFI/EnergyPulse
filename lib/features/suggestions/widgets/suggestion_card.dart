...
class SuggestionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isActionable = suggestion.schedulePayload != null;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                if (isActionable) {
                  _showSetScheduleDialog(context, suggestion, theme);
                } else {
                  _showActionsDialog(context, suggestion, theme);
                }
              },
              ...
              child: Text(isActionable ? 'Set Schedule' : 'Show Actions'),
            ),
          ),
        ],
      ),
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const Expanded(child: Text('Select a Device')),
            ],
          ),
          content: SingleChildScrollView(
            child: ListBody(
              children: DeviceConfig.relayDevices.entries.map((entry) {
                ...
                return ListTile(
                  ...
                  onTap: () async {
                    await _createScheduleForRelay(context, relayId, suggestion);
  ...
  Future<void> _createScheduleForRelay(
    BuildContext context,
    int relayId,
    Suggestion suggestion,
  ) async {
    final payload = suggestion.schedulePayload!;
    final schedule = FirebaseSchedule(
    ...
    try {
      final provider = context.read<FirebaseScheduleProvider>();
      await provider.createSchedule(schedule);

      // Check if the widget is still in the tree before using its context.
      if (!context.mounted) return;

      final deviceName = DeviceConfig.relayDevices[relayId]?.name ?? 'Relay $relayId';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Schedule set for $deviceName!'),
    ...
    onDismiss(); // Dismiss the card after action
  }
